package ru.homework.krestici;


import java.util.Random;
import java.util.Scanner;

public class Krestici {
    ...
    private static void aiTurn () {
        for (int i = 0; winLenght - i > 2; i++) {
            if (scanField(DOT_AI, winLenght - i)) return;   // проверка выигрыша компа
            if (scanField(DOT_HUMAN, winLenght - i)) return;  // проверка выигрыша человека
        }
        aiTurnEasy();
    }
    ...
    private static boolean  scanField (char dot, int lenght) {
        for (int y = 0; y < fieldSizeY; y++) {
            for (int x = 0; x < fieldSizeX; x++) {
                if (cellEmpty(y, x)) {
                    field[y][x] = dot;    //если ячейка пустая, то ставим фишку на которую мы проверяем
                    if (checkWin(dot, lenght)) {   // если в этой точке мы видим что будет чья-то победа то ...
                        if (dot == DOT_AI) return true; // если АИ видит что может поставить и выиграть - то ставит
                        if (dot == DOT_HUMAN) {          // если АИ видит что Чел может выиграть поставив туда свою фишку
                            field [y][x] =DOT_AI;      // то он ей присваивает себя
                            return true;
                        }
                    }
                    field [y][x] =DOT_EMPTY;    // если ничего такого нет, то клетка снова становится пустой
                }
            }
        }
        return false;
